...
app = Flask(__name__)
app.secret_key = os.environ.get('SECRET_KEY', 'dropship-ceo-secret-2024')

DATA_DIR = os.environ.get('DATA_DIR', '/data')
os.makedirs(DATA_DIR, exist_ok=True)

# ============== DATABASE ==============
DB_FILE = os.path.join(DATA_DIR, 'dropship_ceo.db')
...
def ceo_analyze():
    ...
    return jsonify({
        'analysis': analysis,
        'stats': {
            'total_orders': len(orders), 'total_products': len(products),
...
def admin_ai_settings():
    ...
    if request.method == 'POST':
        ...
        db.execute('INSERT OR REPLACE INTO admin_api_keys (id, groq_key, qwen_key, active_provider, updated_at) VALUES (1, ?, ?, ?, CURRENT_TIMESTAMP)', (groq_key, qwen_key, active_provider))
        db.commit()
...
def my_settings():
    ...
    if request.method == 'POST':
        ...
        qwen_key = request.form.get('qwen_key', '').strip()
        active_provider = request.form.get('active_provider', 'qwen')
        db.execute('INSERT OR REPLACE INTO user_api_keys (user_id, groq_key, qwen_key, active_provider, updated_at) VALUES (?, ?, ?, ?, CURRENT_TIMESTAMP)', (user_id, groq_key, qwen_key, active_provider))
        db.commit()
